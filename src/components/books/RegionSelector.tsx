        ...
        {ETHIOPIAN_REGIONS.map((reg) => {
          ...
          return (
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap transition-all flex items-center gap-1.5 ${
                isSelected
                  ? `${reg.badgeColor} shadow-sm ring-2 ring-offset-1 ring-blue-500`
                  : 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700'
              }`}
            >
        ...
        <button
          ...
          className={`p-3 rounded-2xl text-left border text-xs font-semibold transition-all relative ${
            selectedRegion === 'all'
              ...
              : 'bg-slate-50 dark:bg-slate-800/50 text-slate-700 dark:text-slate-300 border-slate-200 dark:border-slate-800 hover:border-blue-400'
          }`}
        >
          ...
          {selectedRegion === 'all' && (
            <CheckCircle className="w-3.5 h-3.5 absolute top-2 right-2 text-sky-400" />
          )}
        </button>
          ...
          return (
            <button
              ...
              className={`p-3 rounded-2xl text-left border text-xs font-medium transition-all relative ${
                isSelected
                  ? `${reg.badgeColor} border-transparent shadow-md ring-2 ring-offset-2 ring-blue-500`
                  : 'bg-slate-50 dark:bg-slate-800/50 text-slate-800 dark:text-slate-200 border-slate-200 dark:border-slate-800 hover:border-blue-400 hover:bg-slate-100 dark:hover:bg-slate-800'
              }`}
            >
